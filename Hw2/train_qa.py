...
class DataTrainingArguments:
    ...
    context_file: Optional[str] = field(
        default=None,
        metadata={"help": "An input context data file to map relevant context."},
    )
    ...
    if data_args.dataset_name is not None:
        ...
        def preprocess_data_file(file_path):
            ...
            for _, row in enumerate(data):
                row["relevant"] = context_data[row["relevant"]]
        ...
        if data_args.context_file is not None:
            with open(data_args.context_file, newline='', encoding='utf-8') as jsonfile:
                context_data = json.load(jsonfile)
        raw_datasets = {}
        if data_args.train_file is not None:
            raw_datasets["train"] = preprocess_data_file(data_args.train_file)
        if data_args.validation_file is not None:
            raw_datasets["validation"] = preprocess_data_file(data_args.validation_file)
